...
export function ChatMessages({
  ...
  const scrollToTop = () => {
    if (messages.length > 0) virtualizer.scrollToIndex(0, { align: "start" });
  };
  ...
  return (
    <div className="relative flex-1 overflow-hidden">
      <div ref={chatScrollRef} className="h-full overflow-y-auto px-5 py-4">
      ...
      {messages.length > 5 && (
        <div className="absolute bottom-4 right-4 z-10 flex flex-col gap-1.5">
          <button
            onClick={scrollToTop}
            className={cn(
              "flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-md border border-[#e2ddd6] text-[#72706a] transition-all hover:bg-[#f5f3ee]",
              atTop ? "opacity-30 pointer-events-none" : "opacity-100"
            )}
            aria-label="맨 위로"
          >
          ...
          <button
            ...
            className={cn(
              "flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-md border border-[#e2ddd6] text-[#72706a] transition-all hover:bg-[#f5f3ee]",
              atBottom ? "opacity-30 pointer-events-none" : "opacity-100"
            )}
            aria-label="맨 아래로"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
