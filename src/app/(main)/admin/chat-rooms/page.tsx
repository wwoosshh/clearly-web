  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="show">
      <motion.div variants={fadeUp}>
        <h1 className="text-xl font-bold text-[#141412]">채팅 모니터링</h1>
        <p className="mt-1 text-sm text-[#72706a]">전체 채팅방을 모니터링합니다.</p>
      </motion.div>
              ...
              <table className="w-full text-left text-sm min-w-[700px]">
                <thead>
                  <tr className="border-b border-[#e2ddd6] bg-[#f0ede8]">
                    ...
                    <th className="px-4 py-3 text-[12px] font-semibold text-[#72706a]">마지막 메시지</th>
                    <th className="px-4 py-3 text-[12px] font-semibold text-[#72706a]">메시지 수</th>
                    <th className="px-4 py-3 text-[12px] font-semibold text-[#72706a]">상태</th>
                    <th className="px-4 py-3 text-[12px] font-semibold text-[#72706a]">최근 활동</th>
                ...
                <tbody>
                  {chatRooms.length === 0 ? (
                    ...
                    chatRooms.map((room) => (
                      <tr key={room.id} className="border-b border-[#e2ddd6] last:border-0 hover:bg-[#f5f3ee]">
                        <td className="px-4 py-3 text-[13px] text-[#1a1918]">{room.user?.name || "-"}</td>
                        <td className="px-4 py-3 text-[13px] text-[#1a1918]">{room.company?.businessName || "-"}</td>
                        <td className="max-w-[200px] px-4 py-3 text-[12px] text-[#72706a] truncate">
                          {room.messages?.[0]?.content || room.lastMessage || "-"}
                        </td>
                        <td className="px-4 py-3 text-[13px] text-[#72706a]">{room._count?.messages || 0}</td>
